...
class BettiCurve:
    ...
    def __call__(self, threshold):
        ...
        self._data.index = pd.to_numeric(self._data.index, errors='coerce') #
        index_values = self._data.index.values #
        match_indices = np.searchsorted(index_values.astype(threshold.dtype), threshold) #
        match = self._data[match_indices]
        
        if not match.empty:
            # return match.values[0]
            return [mv for mv in match.values]
    ...
    def std_thresholds(self, num_thresholds=50):
        min_threshold = min(self._data.index)
        max_threshold = max(self._data.index)

        std_thresholds = np.linspace(min_threshold, max_threshold, num=num_thresholds)
